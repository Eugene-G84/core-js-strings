...
function removeFirstOccurrences(str, value) {
  ...
  const point = index + value.length;
  let res = '';

  if (index >= 0) {
    for (let i = 0; i < str.length; i += 1) {
      if (i >= index && i < point) res += '';
      else res += str[i];
    }
  } else {
    return str;
...
function formatTime(minutes, seconds) {
  ...
  if (minutes >= 0 && minutes < 10) min = min.padStart(2, '0');
  if (seconds >= 0 && seconds < 10) sec = sec.padStart(2, '0');
